...
function CryptoSignals() {
  ...
  const [closeIdCrypto, setCloseIdCrypto] = useState('')
  ...
  const getAllData = () => {
    axios.get(`${url}api/category_signal/getAllCategory_Signals`)
      .then((response) => {
        ...
        console.log(response)
        setCategoryAdd(response.data.result[2]._id)
        // setstockId(categoryAdd)
        const stoxk=response.data.result[2]._id
        const users = response.data.result;
        setCategories(users);
        axios.get(`${url}api/type_category_signal/getTypeCatSignalByCategory_signal_id/${response.data.result[2]._id}`)
          .then((response) => {
            const users = response.data.result;
            ...
            const thirdStock = response.data.result[2]._id

            setCloseIdCrypto(response.data.result[2]._id)
            ...
            axios.get(`${url}api/signal/getSignalsByCategoryId_and_typeCatId/?category_id=${stoxk}&type_cat_id=${longterm}`)
              ...
              .catch(error => console.error(`Error:${error}`));
              axios.get(`${url}api/signal/getSignalsByCategoryId_and_typeCatId/?category_id=${stoxk}&type_cat_id=${thirdStock}`)
              .then((response) => {
                console.log(response)
                setthirdcat(response.data.result);
              })
              .catch(error => console.error(`Error:${error}`));
              // setthirdcat
          ...
          <Grid container spacing={6}>
            <Grid item xs={12}>
              <Card style={{ borderRadius: '10px', backgroundColor: '#202940' }}>
                ...
                {addData ?
                  <MDBox style={{ maxWidth: '100%', padding: '30px', color: 'white' }}>
                        ...
                        <Grid container mt spacing={6}>
                          ...
                          <Grid item xs={12} md={3}>
                            {/* <FormControl fullWidth>
                              ...
                              >
                                 <MenuItem style={{ display: 'flex', color: 'white' }} value="">Company Name</MenuItem>
                                {companies.map((row) => (
                          ...
                          <Grid item xs={12} md={3}>
                            <TextField value={buyTargetAdd} style={{width:"100%"}}
                              ...
                              }   type='number' id="textFieldStyle" label="Enter Buy Target" variant="outlined" />
                          </Grid>
                          <Grid item xs={12} md={3}>
                            <MDTypography color="white"variant="h6" fontWeight="light">
                              Stop Loss *
