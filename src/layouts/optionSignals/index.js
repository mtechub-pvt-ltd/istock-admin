...
function OptionSignal() {
  ...
  const submitHandler = () => {
    if (companyAdd === '' || categoryTypeAdd === "" || buyTargetAdd === "" || stopLossAdd === "" || sellTargetAdd === "" || actualGainAdd === "" || signalNotesAdd === "") {
      setErrorSB(true)
  ...
  const [closeIdOption, setCloseIdOption] = useState(true)
  ...
  const getAllData = () => {
    axios.get(`${url}api/category_signal/getAllCategory_Signals`)
      .then((response) => {
        ...
        setCategoryAdd(response.data.result[1]._id)
        const stoxk = response.data.result[1]._id
        const users = response.data.result;
        setCategories(users);
        axios.get(`${url}api/type_category_signal/getTypeCatSignalByCategory_signal_id/${response.data.result[1]._id}`)
          .then((response) => {
            const users = response.data.result;
            console.log('sfshdjhdjh')
            console.log(response.data.result)
            //close
            setCloseIdOption(response.data.result[1]._id)
            console.log(response.data.result[1]._id)
            const longterm = response.data.result[1]._id
            const swingStock = response.data.result[0]._id
                      ...
                      <Grid item xs={12} md={12}>
                        <Grid container mt spacing={6}>
                          ...
                          <Grid item xs={12} md={3}>
                            ...
                            >
                              {user.map((row) => (
                                <MenuItem style={{ display: 'flex', color: 'white' }} value={row._id}>{row.name==="close"?<></>:<>{row.name}</>}</MenuItem>
                              ))}
                          ...
                          <Grid item xs={12} md={3}>
                            <TextField value={stopLossAdd}
                              onChange={(e) => setStopLossAdd(e.target.value)
                              } id="textFieldStyle" type='number' style={{ width: "100%" }} label="Enter Stop Loss" variant="outlined" />
                          </Grid>
                          <Grid item xs={12} md={3}>
                            <MDTypography color="white" variant="h6" fontWeight="light">
                              Sell Target *
                  ...
                  <Box sx={{ width: '100%', padding: '30px' }}>
                    <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                      <Tabs value={value} onChange={handleChange} aria-label="basic tabs">
                        {user.map((row) => (
                          <Tab style={{ height: '40px', textTransform: "uppercase" }} label={row.name} {...a11yProps(row._id)} />
                        ))}
                      </Tabs>
                    </Box>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={12}>
                        <Grid container mt spacing={6}>
                          ...
                          <Grid item xs={12} md={3}>
                            <TextField value={stopLossEdit}
                              onChange={(e) => setStopLossEdit(e.target.value)
                              } id="textFieldStyle" style={{ width: "100%" }} type='number' label="Enter Stop Loss" variant="outlined" />
                          </Grid>
                          <Grid item xs={12} md={3}>
                            <MDTypography color="white" variant="h6" fontWeight="light">
                              Sell Target *
                          ...
                          <Grid item xs={12} md={3}>
                            <TextField value={sellTargetEdit}
                              onChange={(e) => setSellTargetEdit(e.target.value)
                              } id="textFieldStyle" style={{ width: "100%" }} type='number' label="Enter Sell Target" variant="outlined" />
                          </Grid>
                          <Grid item xs={12} md={3}>
                            <MDTypography color="white" variant="h6" fontWeight="light">
                              Actual Gain *
                          ...
                          <Grid item xs={12} md={3}>
                            <TextField value={actualGainEdit}
                              onChange={(e) => setActualGainEdit(e.target.value)
                              } id="textFieldStyle" style={{ width: "100%" }} type='number' label="Enter Actual Gain" variant="outlined" />
                          </Grid>
                          <Grid item xs={12} md={3}>
                            <MDTypography color="white" variant="h6" fontWeight="light">
                              Signal Notes *
